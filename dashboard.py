...
# df = pd.read_csv('doc/PROJECT/all_dataproject.csv')
...
all_df = pd.read_csv("all_dataproject.csv")
...
#####################################################################################################################
# karena pada function" yang digunakan menggunakan 'return' (nama_variabel), harus kita deklarasi dulu variabel yang 
# punya 'return' diatas, lalu dalam variabel itu akan kita call function nya
...
with col1:
    # fig, ax = plt.sublots(figsize=(20,10)) #############################################################################
    ...
    sns.barplot(
        ...
        y='customer_count', 
        x='gender',
        # data=bygender_df.sort_values(by='customer_count', ascending=False), paletter=colors,
        data=bygender_df.sort_values(by='customer_count', ascending=False), palette=colors, # palleter -> pallete #########
        ax=ax
    )
...
with col2:
    ...
    colors = ["#D3D3D3", "#96CAF9", "#D3D3D3", "#D3D3D3", "#D3D3D3"]
...
fig, ax = plt.subplots(figsize=(20, 10))
colors = ["#96CAF9", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3"]
